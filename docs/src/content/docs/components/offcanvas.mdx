...
Controls the visibility and appearance of the backdrop behind the offcanvas panel. The `backdrop` option has two types:

- **`BackdropVisible`**:
    - **`visibility`**: `"visible"` (required to show the backdrop).
    - **`backdropClass`**: Custom class to style the backdrop (optional).

- **`BackdropHidden`**:
    - **`visibility`**: `"hidden"` (hides the backdrop).
